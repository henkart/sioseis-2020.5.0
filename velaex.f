     ...
     *     IFIN)
!     VELAEX IS THE EXECUTION PHASE OF THE SEISMIC PROCESS VELAN, WHICH IS THE
!  VELOCITY ANALYSIS PROCESS.  THE VELAN PARAMETERS MUST BE ON DISC FILE MUNIT
!  AND THE TRACE (WITH HEADER) MUST BE IN MEMORY LOCATION BUF.
!     SUBROUTINE VELAED CONTANS THE EXPLAINATION OF THE USER PARAMETERS AND THE
!  ORDER OF THE USER PARAMETERS ON DISC.  VELAEX SAVES ALL THE INPUT TRACES ON
!  DISC (WITH HEADERS) BEFORE DOING ANY WORK (THIS IS COMMN TO ALL TYPES OF
!  VELOCITY ANALYSIS), THUS, NREADY=0 ON ALL TRACES INITIALLY.
!
!  ARGUMENTS:
!  BUF    - THE TRACE TO BE ANALIZED, INCLUDING THE TRACE HEADER.  THE FIRST
...
!  PAUL HENKART, SCRIPPS INSTITUTION OF OCEANOGRAPHY, 5 APRIL 1981
...
!
!  mod 14 oct 89 - some velocities were done twice in semblance (those
!                  at the velocity increment change)
!  modified by pch Aug 89 to put the velocity into ibuf(46)
!  modified by GMK December 94 to output semblance file as segy, see edit phase for info
!  mod Oct 95 - don't print the spectra if vtuple(1) is 0
!  mod 6 Nov. 95 - Truncate rather than round nvels on Matlab header.
!  mod 22 Dec 97 - Change MIN0 to MIN
!  mod 13 May 1998 - Subtract 1 from the number of semlances when
!     doing the spline.  Also check for exceeding the spline arrays
!  mod 11 Jan 99 - Change number of argument to nmoapp.  Some 
!    undocumented change in nmoapp!
!  mod 17 Aug 05 - Increase semscr from 2500 to 4000 so output segy
!                  semblance file can be 4000 samples.
!  mod 5 Mar 08 - Take care of endianness for Matlab
!   mod 17 May 10 - Use mute times in mils (ibuf(56 & 57)) rather than 
!                   real seconds (buf(47 & 48))
!  mod 31 Aug 10 - do byte swap for segy opath
!  mod 20 Oct 10 - finish the above
!  mod 13 Jul 11 - Increase semscr from 4000 to 10000
!  mod 9 Apr 13 - remove some print statements.
!  mod 20 Mar 20 - remove FREETP
!  mod 30 Jun 20 - Set IFIN to 0 on first call
!
      ...
      DIMENSION BUF(1111),LBUF(1111),IBUF(1111),
     &          SCR(1111),LSCR(1111),ISCR(1111)
      DIMENSION VTUPLE(3)
...
!***GMK BOOLEAN USED TO CHECK WHETHER EBCDIC & TAPE ID HEADER WAS WRITTEN
      FIRSTSEGY = .TRUE.
!****
...
!      IF(BUF(48).NE.0.) INDX=(BUF(48)-BUF(46))/BUF(49)-1               ! (MUTE-DELAY)/SI
      IF( ibuf(57) .NE.  0 ) 
     &    indx = (FLOAT(ibuf(57))/1000. - buf(46)) / buf(49) -1
      ...
      GO TO 50
   45 DO I=1,ITAPER                                                  ! BUILD THE TAPER IN MEMORY SCR
   ...
   47    BUF(NUMHDR+NSAMPS-I+1)=BUF(NUMHDR+NSAMPS-I+1)*SCR(I)
      ENDDO
!****
!****    COLLECT ALL THE INPUT TRACES FOR THE ANALYSIS INTO DISC FILE IVELU1.
!****  EACH TRACE WILL BE NUMHDR+NSAMPS+1 32 BIT WORDS LONG ON DISC FILE IVELU1.
!****  THE EXTRA WORD IS THE NUMBER OF 32 BIT WORDS IN THE HEADER PLUS
!****  THE NUMBER OF WORDS IN THE ACTUAL TRACE.  THIS EXTRA WORD APPEARS
!****   BEFORE EACH TRACE HEADER.
!****
   50 CONTINUE
!****
!****   IF THE DATA IS TO BE ANALYZED AS A REFRACTOR (TAU-P) REMOVE THE
!****   DELAY BY INSERTING ZEROES IN THE FRONT
!****
      ...
      DO I=1,1000                                                    ! MAKE A BUFFER OF ZEROES
   58    SCR(I)=0.
      ENDDO
   59 CONTINUE
!****
!****
!****   Watch out for traces > 32767
      CALL ushort2long( ibuf(58), itemp )
      IF( itemp .GT. 32767 ) THEN
          PRINT *,'  ***   ERROR  ***  Too many samples for VELAN.'
          PRINT *,' Use DISKIN parameters SET or DECIMF.'
          CALL EXIT
      ENDIF
      NWRDS=NUMHDR+IBUF(58)                                             ! THE NUMBER OF 32 BIT WORDS
!**** NSAMPS IS THE LENGTH OF THE ORIGINAL TRACE, IBUF(58) HAS THE NEW NUMBER
      IOUT=0
      IF(IBUF(15).EQ.2.AND.TYPE.NE.'CVEL') GO TO 70
!**** WRITE THE HEADER LENGTH + DATA LENGTH IN FRONT OF EVERY TRACE
...
!      IF(IINRW.NE.0) CALL FREETP(ILUN,0)                               ! UNASSIGN THE TAPE DRIVE
   ...
   80 CONTINUE
!****
!****     APPLY THE TAPER TO THE FIRST AND LAST NTAPER TRACES OF THE GATHER
!****
   ...
   85 CONTINUE
!****  NOW SKIP THE NON WEIGHTED TRACES
...
!****
!****  THIS IS A REFRACTION VELOCITY ANALYSIS
!****  USE T=T-X/V RATHER THAN T=SQRT(T*T-X*X/V*V)
!****  THIS ALSO TRANSFORMS THE DATA TO THE TAU-P DOMAIN!!!
!****  RATHER THAN ACTUALLY SHIFTING THE DATA, CHANGE THE START TIME OF THE
!****  DATA (THE DELAY) SO THAT THE DATA IS NEVER CUT OFF!
!****
      ...
      IEND=NUMHDR
      GO TO 205
  203 DO I=ISTART,IEND
  204    BUF(I)=BUF(I+NSHIFT)
      ENDDO
  205 CONTINUE
!  205 BUF(47)=BUF(47)-SHIFT                                            ! THE MUTE START TIME
!      IF(BUF(47).LT.0.) BUF(47)=0.
!      IBUF(56)=BUF(47)*1000.+.5                                        ! CONVERT TO MILS
!      BUF(48)=BUF(48)-SHIFT                                            ! CHANGE THE MUTE TIME IN THE HEADER
!      IF(BUF(48).LT.0.) BUF(48)=0.
!      IBUF(57)=BUF(48)*1000.+.5
...
!****  START AT THE BEGINNING OF THE INPUT FILE
 ...
 1040 CONTINUE
      CALL RDDISC(IVELU1,NWRDS,1,ISTAT)
      CALL RDDISC(IVELU1,BUF,NWRDS,ISTAT)
!****  INITIALIZE THE PLOT HERE BECAUSE IT NEEDS THE TRACE HEADER OF THE
!****  TRACE CLOSEST TO THE CENTER OF THE RP.
     ...
     *    CALL INVPLT(VTUPLE,ISCR,IBUF,LBUF,BLANK)
      ...
      CALL APWD
!**** COMPUTE THE CONSTANT VELOCITY NMO ON THE REAL WINDOW CENTER TIMES
...
!****   COMPUTE THE STACK TRACE AND THE MEAN ENERGY
...
!      CALL STKSEM(NEW,ISTKAD+IADD,ISCRAP,ISEMAD+I,ILEN,IONE,IDIVAD+I)
 ...
 1130 CONTINUE
!****
!****  FINISH THE SEMBLANCE CALCULATIONS BY FINDING THE ENERGY OF THE STACKED
!****  TRACE AND DIVIDING IT BY THE MEAN ENERGY.
      IF( IUSEAP .NE. 0 ) THEN
...
!     *       NINE,1)
      ...
      ENDIF
!***GMK TEST TO DETERMINE WHAT TYPE OF FILE< SEGY OR ASCII NEEDS TO BE WRITTEN
      IF( ivelu3 .NE. 0 ) THEN
...
!*** GMK WRITE OUT 3200 BYTES INTO EBCDIC HEADER
!*** WE ARE USING THE LBUF/IBUF/BUF ARRAY TO FORMAT NEW SEMBLANCE TRACE HEADERS
!*** SOME GARBAGE (HOPEFULLY USELESS) WILL BE CARRIED INO NEW HEADER, CAN DO
!*** THIS SINCE PROCESS ENDS WITH VELAN AND DOESN'T NEED TO PRESERVE OLD HEADER 
!*** VALUES.
                      ...
                      ibuf(13) = 5
                      ibuf(15) = 1
!***GMK WRITE OUT TAPE ID HEADER WITH APPROPRIATE VAULES FOR SEMBLANCE
...
!*   Matlab 4 header, word 1, is m0pt, where:
!*  m - thousands digit.  0 -> little endian (PC)
!*                        1 -> big endian
!*  0 - hundreds digit.
!*  p - tens digit.  0 -> 64 bit float.  1-> 32 bit float, 2 -> 32 bit int
!*           3 -> 16 bit int, 4 -> 16 bit unsigned int, 5 -> 8 bit unsigned.
...
!***GMK Spline Interpolate Semblance values back to original time
!***    series length and deltat. Use subroutine splint and spline
!***    from process logsex.f
!***GMK Calculate time value of semblance
...
870           CONTINUE
              YP1=1.0E+31
              YPN=1.0E+31
!***GMK Calculate 2nd derivatives required for splining
...
!***GMK Calculate current semblance value (curyval) for given time (curxval).
...
871           CONTINUE            
     ...
     &                CALL swap32( semscr, nsamps )
          ...
          ENDIF
!            IF( nsegyfile .EQ. 2 ) CALL wrdisc( ivelu3, scr, n )
          ITRACE = ITRACE + 1                        
      ENDIF
      DO 1170 II=1,N
