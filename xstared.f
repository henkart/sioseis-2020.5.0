      SUBROUTINE xstared
!     Process XSTAR converts EdgeTech's XSTAR chirp data into a
!  more useful format.  The enhacements are:
!  0)  Look for pings with only 1 ducer (trace).
!  1)  Perform a complex modulus to convert to amplitude.
!  2)  Change the sample interval and data length to account
!      for the modulus.  (the sample interval is 83 us)
!  3)  Perform a stack of both traces in each shot.
!  4)  Make the shot number appear in the correct SEG-Y location.
!  5)  Make the shot trace number 1.
!  6)  Use the NMEA time for the shot time.
!
! Copyright (C), The Regents of the University of California
! Written by Paul Henkart, Scripps Institution of Oceanography, 2001
!
! mod Dec 2002 - Add parameters ftr, ltr, type
!         type 0 = gstar, 1 = 1 trace xstar, 2 = 2 trace xstar
!                Add parameter weights
! mod 15 Feb 03 - Weights was preset 1 2 rather than 1 1
!               - ltr didn't work
! mod 19 Feb 03 - Add parameter DUMMIES (preset = yes)
! mod 25 Mar 03 - Allow type 3 for the Triton 3 trace data.
! mod 1 Dec  04 - Add parameter MKREAL
! mod 11 Jan 05 - Change dummies into 0, 1, 2 from yes/no and add 2 to
!                 mean use the ping if 1 of the ducers is missing.
! mod 25 Aug 05 - Add type 5 - Xstar version 5
! mod 30 Jun 06 - Allow type 4 (unsummed output).
!
      PARAMETER ( npars = 12 )
...
!****
  ...
  130            PRINT *,' ***  ERROR  ***  Can not find file ',token
     ...
     &                   token
             ...
             GOTO 100
         ENDIF
!****    the token is not in the parameter list.  Might be weights.
