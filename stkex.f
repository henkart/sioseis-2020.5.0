     ...
     &                    istop, nready )
! mod Oct 02 - Trim/median stack bombed when first trace was zero
! mod 11 Mar 03 - Add panel.
! mod Apr 09 - Allow unsigned nsamps
! mod 15 June 09 - Apr 09 was done wrong!
! mmod 12 Apr 10 - Use end mute integer mils rather than real second (word 48)
!
...
!      nsamps = ibuf(isampptr)
      ...
      IF( mlists .EQ. 0 ) THEN
          ...
          IF( nxxts .GT. 0 ) THEN
              ...
              ENDDO
          ENDIF
!****     make sure the "ap" gets initialize
          CALL inap( buf(numhdr+1), nsamps )
      ENDIF
      iout = 0                                                          ! force the data out of the ap
      CALL rlseap( buf(numhdr+1), nsamps )
!****
!****  Collect all the traces on disk until End-of-Gather or End-of-Job
!****
      nwrds = nsamps + numhdr
      ...
      ENDIF
!****
!**** A stack panel is a progressive stack.  Each input trace of the
!**** gather is added to the stack and the current stacked trace is
!**** output.  Let's do the entire sum for each new trace so that
!**** things like mutes can be honored.
!****
      IF( panel .NE. 0. ) THEN
!****     The summed output goes in buf.
!****     The trace from disk goes in scr.
!****     The divisor goes in apdata
!****     let's assume all traces in the gather have the same trace
!****     length and delay.  Should/could check, but ....
!****     So nsamps is still good!
          DO i = 1, nsamps
             buf(numhdr+i) = 0.
             apdata(nextad+i-1) = 0.
          ENDDO
...
!             muteend = NINT((scr(48)-delay) / si)
      ...
      ENDIF
!****
!**** Do the whole dataset, make sure everything has the same delay 
!**** and length.  Start by adding the pads.
!****
   90 CONTINUE
      CALL podisc( luntrim, 0, 0 )
      nsamps = last - mindel+1
!****
!****  trim stack here  (save inner traces or save outer traces)
!****
...
!****        make every trace start at the same time
             ...
             index = index + npts
!****        make sure every trace ends at the same time
!****          last is from 1, as if no delay
             n = last - npts
             IF( n .GT. 0 ) THEN
                 DO j = 1, n
                    apdata(index+j) = 0.
 ...
 1000        CONTINUE
      ...
      ENDIF
!****
!****   XXT here - inner/outer muting
!****
!	 print *,' nxxts=',nxxts,' xxt=',(xxt(i),i=1,6)
      IF( nxxts .GT. 0 ) THEN
          DO i = 1, last
...
!	 print *,' si=',si,' nsamps=',nsamps,' idelay=',idelay
             CALL rddisc( luntrim, buf, nsamps, istat )
             indexo = idelay - mindel
             indexap = nextad + indexo
             ixxt = 1
!	 print *,' input=',(buf(j),j=1,10)
             DO j = 1, nsamps
                time = FLOAT(j-1)*si + delay
!	 print *,' time=',time,' ixxt=',ixxt,' xxt=',xxt(ixxt+2)
...
!****
!****
!**** Do the trace header before leaving
!****
      DO i = 1, 60
         buf(i) = trhdr(i)
      ENDDO
      ...
      lbuf(10) = 0
      lbuf(15) = 1
!****  the fold is not ntraces, but worry about it later
