...
!
!  mod 24 Jan 92 - Allow successive lists to have the SAME fno/lno.
!  mod 16 Aug 92 - VMS didn't work becuase of disk I/O in EBCDIC header
!  mod 19 Oct 92 - Add parameter HEADER.
!  mod 15 Mar 95 - Add parameter BHDR
!  mod 16 Mar 95 - Add parameter XN1
!  mod 22 May 95 - Correct bugs with ihdr/lhdr/hdr when BHDR was done.
!  mod 10 Jun 95 - Add parameter CLEAN
!  mod 5 Apr 96  - Don't write to header unit if process header parameters
!                  precede the parameters for an input process such as diskin.
!  mod 23 Oct 96 - Added error check for bad parameter HEADER
!  mod 23 Dec 96 - Allow math operations on up to 10 header values.
!  mod 11 Mar 98 - Don't do BHDR if header is NOT in the procs list
!  mod 27 Jun 00 - HDR converted it to integer.
!  mod 20 June 2001 - Allow multiple header processes and make this reentrant.
!  mod 11 Mar 03 - Create parameter INTERP and make it the same as SPAT.
!                - Make the preset for INTERP  NO  when doing equations.
!  mod 6 May 03 - Add parameter REV1 PATHNAME
!  mod 2 June 03 - Only do the binary header stuff if the proper process
!                  HEADER is in the procs list.
!                - variable itype was being used for too many things!
!  mod 27 Apr 06 - BHDR didn't work on Linux because wrdiscb had extra arg
!                - gfortran (linux) doesn't allow GOTO into differnt blocks.
!  mod 25 May 06 - Allow negative fno and lno.
!  mod 24 Jul 06 - Change lno preset to 9999987
...
!  mod 8 Jan 09 - Reel header changes (CXX) didn't work on HEADER2 & HEADER3
!  mod 6 Jun 11 - swap i109 r60 = i109  choked because r60 looks like a swap index
!                 No cure if the r06 is and I or L
!  mod 7 Sep 12 - Use REAL*8 when reading LHDR or L*
!  mod 5 Feb 14 - READ statement had an extra ")" that new compiler choked on
!  mod 14 Apr 14 - add B (byte) to "equation"
!                - Add D (double or 64 bit) to swap
!                - add D (double) to "equation"
!  mod 4 Sep 14 - R on right hand side of equation failed.
!
...
!
      ...
      DIMENSION scr(1111), lscr(1111), ivms(20), iswap(maxswap)         ! 493 = 13+120+120+120+120
...
!****
!****  Get the SEGY header (which is on disk in EBCDIC)
!****    the VMS version of diskio can't read into characetr arrays
!****
      IF( iunhdr .GT. 0 ) THEN
...
!****
  ...
  200 CONTINUE
     ...
     &and < 121.'
                  ...
                  GOTO 100
!                  nswap = nswap -1
!                  PRINT *,' ***  ERROR  ***  SWAP indices must start wit
!     &h I or L.'
!                  ierror = ierror + 1
              ENDIF
          ENDIF
...
!    constant not allowed on the left hand side of an equation!
...
!    a constant doesn't have a letter!  jtupe = 4
...
!****             We have a problem Houston.  This was a constant.  It
!****             may or may not be the last part of the equation.
!****             The next thing must be an operation (+,-,*,/) or else
!****             it's the end of the equation.
                  CALL getoke( token, nchars )
                  CALL upcase( token, nchars )
...
!****
     ...
     &              hdr(i)
       ...
       ENDDO
!****
!****  INTERP/SPAT preset is on except when equations are used.  And,
!****  SPAT should have been named INTERP and I have to leave it in
!****  so old scripts will work.  sh!@#^&*
       IF( ispat .LT. 0 ) THEN
           IF( interp .LT. 0 ) THEN
               interp = 1
...
!****
!**** Write the SEGY header back to disc, if it's been read!
!****
      IF( iunhdr .GT. 0 ) THEN
          DO i = 1, num
