...
!     MIXEX IS THE EXECUTION PHASE OF THE SEISMIC PROCESS MIX, WHICH MIXES
!  SEVERAL ADJACENT TRACES.  THE MIX PARAMETERS MUST BE ON DISC FILE MUNIT
!  AND THE TRACE (WITH HEADER) MUST BE IN MEMORY LOCATION BUF.
!     SUBROUTINE MIXED CONTANS THE EXPLAINATION OF THE USER PARAMETERS AND THE
!  ORDER OF THE USER PARAMETERS ON DISC.
!
!  ARGUMENTS:
!  BUF    - THE TRACE TO BE MIXED, INCLUDING THE TRACE HEADER.  THE FIRST
...
!  PAUL HENKART, SCRIPPS INSTITUTION OF OCEANOGRAPHY, 12 MARCH 1981
...
!  MODS:
!  22 March 1991 by pch to add the Type parameter and record mix
!      and add argument nready
!  7 July 1992 by pch to honor lno
!  13 July 93 by pch to ignore the mute times if they are less than the
!       delay (which will screw up the buffer indices, but can only
!       happen if the mute times are in error anyway!)
!  mod 22 Dec 97 - Change MIN0 to MIN and MAX0 to MAX
!  mod 20 Mar 98 - Allow very large delay (in samples) by preseting
!                  mindel to 1000000 from 10000
!  mod 27 Jun 00 - Add HDR, LHDR, IHDR
!  mod 16 Nov 00 - Above change caused ndone to be incremented at
!                  the wrong place, which then ended up causing
!                  an index to go bad and segmentation fault
!
!
...
!      ITABLE IS COMPOSED OF 10 ROWS, ONE FOR EACH TRACE TO BE MIXED, AND
!      6 COLUMNS, WHERE
!      COLUMN 1 = THE TRACE NUMBER WITHIN THE MIX.
!      COLUMN 2 = THE AP ADDRESS OF THE TRACE. A NEGATIVE ADDRESS MEANS THERE IS
!                 NOT A TRACE IN THE AP AT THE ADDRESS.
!      COLUMN 3 = THE TRACE ID.  =2 MEANS A DEAD TRACE.
!      COLUMN 4 = THE DELAY, IN SAMPLES, OF THE FIRST DATA SAMPLE.
!      COLUMN 5 = THE NUMBER OF SAMPLES IN THE TRACE.
!      COLUMN 6 = THE MUTE, IN SAMPLES (FROM ZERO TIME)
...
!             MINDEL=MIN0(MINDEL,ITABLE(I,4))
             MINDEL=MIN(MINDEL,ITABLE(I,4))
!             MAXSA=MAX0(MAXSA,ITABLE(I,5))
