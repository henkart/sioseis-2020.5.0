...
!     SPP simulates the Versatec Simultaneous Print/Plot mode by ORing a
!  character arry into a set of raster lines.
!     The way this program works is that it has only numeric fonts.  When
!  SPP gets a non blank annotation (c), it moves 16 lines of digitized
!  font into a storage buffer.  Each call to SPP then ORs the top line of
!  the storage buffer into the users ibuf array.  After the OR, the storage
!  array is moved up one raster so that the next call to SPP gets in essence
!  the next line of the character.  Thus, it takes 16 calls to SPP to flush
!  a whole character.  A new non blank character in c before the 16th call
!  will reset the SPP OR array.
!     This version of SPP assumes c is 8 characters big.
!
!  Copyright (C) 1987 by Paul Henkart, Seismic Reflection Processors, San Diego
!  ALL RIGHTS RESERVED
!
!   mod 8 Aug 1991 by pch - add minus
...
! mod 10 Jul 02 - Add reverse direction annotation by using kfonts and lfonts,
!                which is upsidedown and backwards.
! mod 29 Jan 03 - nibs 75 annotation on PC was bad
! mod 13 Jan 04 - Change common defs and colors from 8 to 9
! mod 10 Oct 06 - Change ltr font (kfonts) for 1 and 3
! mod 20 Aug 08 - Added i255 for g95
!
      INTEGER*2 ibuf(1111),itemp,itemp1
   ...
   20            minus(i) = NOT( minus(i) )
      ...
      IF( cc(1:8) .NE. ' ') THEN
          ...
          DO 190 i=1,8                                                  ! do each character
              IF( cc(i:i) .NE. ' ') THEN
!                                   ICHAR('0') is 48 (decimal or 30 hex)
  ...
  190     CONTINUE
      ...
      IF( rnibs .GT. 100. ) THEN
          DO 200 i=1,8
!             ibuf(i) = IOR( ibuf(i), iorbuf(i,1) )                       ! only 1 raster line!
             ibuf(i) = iorbuf(i,1)
  200     CONTINUE
      ELSE
          j = 1
