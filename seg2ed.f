...
!                       ------- ------
!
!  Document date: 17 August 1998
!
!  little endian - little end first - DEC and INTEL
!
...
!  ALL RIGHTS RESERVED.
!
! mod 26 Apr 01,  If byte swapping, do the data format code also
! Also, don't put the sample interval in the header since it's not known yet.
! mod 1 June 01 - oops, that change made ifmt bad
! mod 29 Sep 08 - FFILEN no longer worked (I4 now gives leading blanks)
! mod 21 Mar 09 - Allow 5 digits! (I5)
!
      PARAMETER ( nmulti = 0 )
...
!          ifmt = ibuf(7) / 256
      ...
      ENDIF
!****
!****   May need to get a trace descriptor in order to get the sample interval!
!****
...
!****
!****   create a trace header and leave
!****
      DO i = 1, numhdr
         lbuf(i) = 0
      ENDDO
      ibuf(58) = numdat
!     si hasn't been set yet.
!      ibuf(59) = NINT(si*100000.)
!      buf(49) = si
      RETURN
      END
