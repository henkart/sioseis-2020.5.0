      SUBROUTINE setptr
!****    Set the pointers or indices of the SEGY trace header.
!****  The problem with using constants is that the SEGY header
!****  contains 16 and 32 bit integers, wherease ANSII Fortran 77
!****  only knows INTEGER, which happens to be INTEGER*8 on the
!****  Cray!  Most routines don't reaaly need to know the
!****  difference.  On the Cray, the header is expanded into
!****  a 200 word header and the 16 bit integer values are moved.
!****
!****  when adding something to common, add it to the end so that all
!****  subroutines don't have to be recompiled!
