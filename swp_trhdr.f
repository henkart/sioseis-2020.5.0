...
!**** Do the segy trace header byte swap.
!**** This routine does not care what endian the header is nor what endian the computer is.
!**** This routine does take into account what segy rev the header is though.
!
! Rev 0 had bytes 181-240 as unassigned and sioseis used it for REAL entries, thus used swap32
! Knudsen Rev 0 (that's all I've seen) use 181-240 for INTEGER*2, but sioseis doesn'r use them.
! Rev 1 bytes 181-240 are mixed.  Sioseis only uses the deep water delay scalar 
!     in bytes 215-216 (ibuf(108))
!
!   Written April 2014 and thus not used before sioseis ver 2014.4
!
      INTEGER*2 ibuf(*)
      INTEGER*4 lbuf(*)
