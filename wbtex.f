...
!     WBTEX PUTS THE USER DEFINED WATER BOTTOM TIME IN TRACE HEADER WORD 50.
!  WBTEX IS THE EXECUTION PHASE OF THE SEISMIC REFLECTION PROCESS WBT.  THE
!  TIME IN THE HEADER IS IN FLOATING POINT SECONDS.  THE USER'S WATER BOTTOM
!  TIMES MUST BE IN DISC FILE IWUNIT (SEE SUBROUTINE WBTED) AND MUST BE BY RP
!  NUMBERS AND THOSE NUMBERS MUST BE INCREASING ORDER WITHIN THHE DISC FILE.
!
!  PAUL HENKART, SCRIPPS INSTITUTION OF OCEANOGRAPHY, MARCH 1980
!  SOME OF THE AWFUL GYRATIONS WITH MULTIPLYING THE DAY/GMT IS DUE TO A
!  COMPILER BUG ON THE PRIME.
!  mod 24 July 90 to make the rp numbers and gmt floating point so that
!      smoother interpolation can be done.
!  mod 2 aug 90 - remove the prime gyrations discussed above!
!    mod 21 Sep 95 - Add SEL, SES, SOLRAT - an auto picker.
!                  - Remove TYPE
!  mod 29 Jan 96 - Add (ses-delay) to the time of the pick!
!  mod 7 Feb 96 - Convert GMT to decimal minute before interpolation
!  mod 24 Apr 96 - Do not modify trace header delay when auto picking!
!  mod 11 June 96 - Was using lrptrptr rather than lrpnptr!  When was
!                 that change made?  Damn
!  mod 6 June 97 - Add threshold picking
!  mod 22 Oct 98 - Add PEAK picker
!  mod May 99 - Add SEPP (start and end time peak picker)
!  mod 26 Apr 00 - PRESTK didn't work with THRES or PEAK
!  mod June 00 - g77 doesn't like arithmetic inside FLOAT
!  mod 21 Jun 00 - Added INDEX
!  mod June 01 - Add numwbt and make it re-enterable.
!  mod 31 Jul 01 - On THRES, use the time of the last good pick
!                  if this pick doesn't happen.
!  mod 3 Aug 01 - Add parameter TRACK to SEPP and THRES
!  mod 8 Jan 03 - Change slave.f to skip the short window if it's 0.
!  mod 9 Sep 05 - Honor the scalar in segy bytes 69-70 (word 35)
!               - Eliminate old Seabeam scanning by making NSCAN and VEL needed.
!               - VEL only mean simply convert depth to time using vel.
!  mod 22 Feb 08 - Honor track when doing VEL.
!  mod 23 Jun 08 - When doing vel, check for unrealistic depths.
!  mod 27 Jun 08 - When doing vel, check for byte swapped water depth (Knudsen).
!  mod 18 Nov 08 - When doing vel, kill the trace if we haven't seen a non-zero
!                  depth.  (o.w. use last non-zero depth).
!  mod 3 Sep 09 - When vel and track, count nonzero and determine bad only
!                 after 5(?) nozero depths have occurred.
!  mod 5 Oct 09 - Add offline bandpass filtering.
!  mod 25 Nov 09 - Change prestk to be shots as well as rps.
!                - Do the prestk logic early so that it applies to all methods
!  mod 31 Mar 10 - get rid of arithmetic IF
!  mod 13 Aug 10 - The offline filter changes caused procs filter wbt to be bad.
!  mod 8 Jun 12 - Honor prestk when using vel.
!  mod 10 Jul 14 - Add guided & seg
!  mod 7 Oct 14 - get si from common rather than trace header
!  mod 20 Oct 14 - get delay from common since rev 1 uses a scalar
!
      PARAMETER (max = 100)
      PARAMETER (MAXWBTS=3)
      PARAMETER (NPTS = 55)
...
!     &           REAL(ibuf(81) * 60. + ibuf(82)) + REAL(ibuf(83))/60.
...
!    INTERPOLATE THE TIME IF BETWEEN TWO GIVEN BY THE USER
      IF( lday(numwbt) .EQ. 0 ) THEN
!         g77 didn't like the FLOATs on the next statement because
!         l, l1 and l2 are already REAL
!          TIME = FLOAT(L-L1)/FLOAT(L2-L1)*(TIME2-TIME1)+TIME1
  ...
  410 TIME=TIME2                                                        ! USE THE LAST TIME
      GO TO 90
!****
!****      SIO SeaBeam water bottom depth
!****
 1000 CONTINUE
      ...
      ENDIF
!      depth = ibuf(107)
!     a zero depth means we're hard aground
      IF( depth .EQ. 0 ) THEN
!****     kill the trace if there haven't been any good depths yet
          IF( depth1 .EQ. 0 ) THEN
...
!****   Knudsen KEB files have a byte swap bug, so see if that's the problem
          ltemp = lbuf(16)
          CALL swap32(ltemp,1)
          IF( temp .LT. 0 ) THEN
          ...
          ENDIF
      ENDIF
!****   When prestack, use the depth from the first trace for all others in the shot/rp
     ...
     &        depth = depth1
      ENDIF
!****  Knudsen also gives zero when it can't find the water botton.
!****  Use the last good one if this is zero.
      depth1 = depth
 ...
 1010        sbdep(i) = 99999.
          ...
          iaddr(1) = 1
!****  This was i = 1, nscane, but that wrote over iaddr(1)
          DO 1030 i = 2, nscan2
 ...
 1050 CONTINUE
     ...
     &        buf(jndex(numwbt))
      ENDIF
      DO i = 2, nscan2                                             ! rotate the disk address table
 1100    iaddr(i-1) = iaddr(i)
      ENDDO
      iaddr(nscan2) = laddr
      IF( ndone .GE. nscan(numwbt) ) THEN
          DO i = 2, nscan(numwbt)+loffst(numwbt)
...
!      si = buf(lsisptr)
      ...
      IF( lbuf(7) .NE. 0 ) THEN
          ...
          itrno = lbuf(7)
      ENDIF
!     if prestack and this trace belongs to the same rp/shot as the last
!     then use the wbt from the previous trace!
!     index was saved! delay and si may have changed!
      IF( prestk(numwbt) .NE. 0 ) THEN
      ...
      ENDIF
!****
!****  Put all the data into a(nextad) then filter it if requested
!****
!**** force use of the "ap" so it's memory gets allocated
      CALL inap( buf(numhdr+1), numdat )
!**** get the data out of the ap into scr array a(nextad)
      iout = 0
      CALL rlseap( a(nextad), numdat )
!**** that's a problem.  rlseap sets the data to not being in the ap so the
!**** next process thinks the data is in buf, but's it's not since we put
!**** it in a(nextad).  Put it back in a(in).
...
!****
 ...
 2005 CONTINUE
!      IF( in .NE. 0 ) THEN
!          CALL slave( a(in+istartl-1), nlong, a(in+istarts+iadds-1),
!     &        nshort, numdat, sol(numwbt), index, aves, avel, jprint )
!      ELSE
!          CALL slave( buf(numhdr+istartl), nlong, 
!     &                buf(numhdr+istarts+iadds), nshort,
...
!      ENDIF
 ...
 2010 IF( index .GT. 0 ) THEN
     ...
     &            time .LE. picklast(numwbt) + track(numwbt) ) THEN
                  ...
                  RETURN
          ENDIF
!**** If the pick is before the track window, try the next pick by
!**** advancing the short window to the current.
          IF( time .LE. picklast(numwbt) ) THEN
              iadds = iadds + index - nshort/2
              GOTO 2005
          ENDIF
!**** If we get here, then the pick is past the tracking window, so
!**** use the last good pick.
     ...
     &        ' is outside the track.'
      ...
      RETURN
!****
!****   Pick it if it exceeds a threshold
!****
 3000 CONTINUE
...
!                 i = numdat
 ...
 3010     CONTINUE
          ...
          ELSE
              time = delay + pick*si
              IF( picklast(numwbt) .EQ. -99. ) picklast(numwbt) = time
!****         see if this pick is within the track of the last pick
              IF( time .GE. picklast(numwbt) - track(numwbt) .AND.
     &            time .LE. picklast(numwbt) + track(numwbt) ) THEN
                  picktime(numwbt) = time
              ELSE
...
!**** absolute value peak
      IF( peak(numwbt) .EQ. 3. ) THEN
          ...
          ENDDO
      ENDIF
!**** Don't honor the pick if it was the first sample
 ...
 4500 FORMAT (' no ',I6,' trace ',I4,' picked time is ',F8.6,
     &  ' peak is ', G20.10)
      RETURN
!****
!****  Guided - the peak or trough within window SEG which is centered around the 
!****           water bottom time in buf(jndex(numwbt))
!****
 5000 CONTINUE
!**** this is similar to 4000 above
      IF( first(numwbt) ) THEN
          first(numwbt) = .FALSE.
          picklast(numwbt) = -99.
      ENDIF
