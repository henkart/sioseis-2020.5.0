...
!
!
!  COPYRIGHT (C), PAUL HENKART, SCRIPPS INSTITUTION OF OCEANOGRAPHY, AUGUST 1983
...
!
!
!  mod 15 July 90 - add anntyp 9 (GMT+rp)
!  mod 1 May 90 - Make Apollo a Unix machine
!  mod 18 Apr 90 - add the Epson "rugged Writer 480" (nibs=120)
!  mod 5 Apr 90 - use -wiggle when dir = l2r
!  mod 23 jan 90 - add the Versatec 7424 plotter
!  mod 16 Nov 89 - make the plot header blank if column 1 is not ASCII
!  mod 14 Nov 89 - add anntyp 8 (espn)
!  mod 3 Nov 89 - add the Calcomp 5845 plotter.
!  mod 6 feb 91 - remove the CHAR(15) from character 80 when a Versatec
!  mod 6 Aug 91 - add frange, lrange, srpath, sipath
!  mod 14 Aug 91 - Add Versatec 9242 and 8242
!  mod 20 Aug 91 - Add rstime
!  mod 22 Nov 91 - Add Versatec 7436 (Larry Mayer)
!  mod 30 Jan 92 - Change stime preset from -1. to -99999. to allow stime -1
!  mod 4 Feb 92 - Add parameters absval, ann, hdr, ihdr, lhdr.
!  mod 24 Feb 92 - Add the local time to the plot header.
!  mod 20 Mar 92 - Add the Raytheon 850 and the HP 7600
!  mod 14 May 92 - Add color
!  mod 30 Sep 92 - add NovaJet, 300 dpi, 36 in., Color inkjet plotter
!  mod 12 Oct 92 - add HP 2848, 300 dpi, 36 in., B&W inkjet plotter
!  mod 2 Nov 92 - add HP 2847, 300 dpi, 24 in., B&W inkjet plotter
!  mod 4 Feb 93 - The !@#$%^&* Versatec 3444 is CYM not RGB
!               - add the Versatec 3444
!  mod 26 Mar 93 - Add common ver for print the SIOSEIS Version
!  mod 20 Apr 94 - Add the OYO GS-624 plotter
!  mod 3 May 94 by gk - Allow 45 seconds of data to be plotted (was 30).
!  mod ????      - Do not create plot file if plot is not in procs
!  mod 10 Nov 94 - Correct bcolor and colors for 'BLACK' and 'WHITE'
!  mod 30 Nov 94 - Add screen plotter (nibs 75)
!  mod 13 Feb 95 - Write nibs value on line 1 of sioseis plot file.
!                - add nibs 3436 (Versatec), 9800, 9315
!  mod 24 Feb 95 - Change icdots preset to 1
!                - Add parameter tlann
!  7 June 95 - Change nibs 300 from NovaJet to HP color LaserJet
!              (NovaJet should be the same as the HP DesignJet)
!  4 Jan 96 - DesignJet (any 300 dpi) should have vscale at .41666 or .42666
!  12 Feb 96 - flip color order and def and negate def when dir = ltr
!  23 Oct 97 - Add parameter WRAP
!  17 Apr 98 - Increase defs and colors from 6 to 8
!  4 June 98 - Add error check for decreasing tlines.
!  11 Nov 98 - Add annotation type sh&tr and rp&tr
!  8 Nov 98 - Add the Alantec 2400R (23,52in, 200dpi, 588 bytes)
!  19 Apr 99 - Change ASCII format size of trpin and vscale so
!              they don't overflow and confuse program overlay
!  27 Oct 99 - Allow color gray - for gray scale or variable density.
!  24 Sep 00 - Add parameter RECSP
!  2 Mar 01 - The plotter size on nibs 200 (Versatec 1200) is 10.56 not 11.
! 30 Mar 01 - Add parameter HPATH - Header file output
! 4 May 01 - Change format of vscale in plot header to allow vscale > 99
!  15 Jan 02 - Linux was choking with the null on opath and hpath
!  14 Oct 02 - Add parameters ANNTYP2, ANN2 and RECTIFY.
!  31 Jan 03 - colors gray needed the deflections to be * 2.
!  7 Feb 03 - Do error check for deflection-graylevel pair order.
!           - Add parameter dptr (dots per trace).
!  24 Feb 03 - Make new paramter NOINC equivalent to NINC.
!  13 Jan 04 - Increase defs and colors to 9 in common
!  14 Apr 04 - Add nibs 2368 - iSys V12
!  11 May 06 - Don't do nsecs 0 in ploted since diskin might change nsamps
!            - Change nsecs preset to 0
!  14 Mar 07 - Add CHART
!  5 April 07 - Add PLOTTER and make NIBS = PLOTTER
!  27 Apr. 07 - Make HPATH a SEG-Y file by adding the ASCII and binary headers.
!  19 July 07 - Change offset to 0 for iSys (2368) plotter.
!  23 July 07 -  Add size to common/seispl/
!  14 Sep 07 - Add some error checks for chart
!  2 Oct 07 - colors didn't work because an undocumented change was bad!
!  7 Dec 07 - Add parameter TRIM
!  22 Apr 08 - Change nibs preset to 2959
!  4 Aug 08 - Add ann/ann2 lat/log
!  28 Aug 09 - Add SECS and make it same as NSECS
!  29 Apr 10 - Add 24 and 44 inch HP Z2100 as 2124 and 2144
!  22 Jul 10 - Add lat100 and long100 to ann and ann2
!  15 Oct 10 - Remove all the warnings about uneven vscale and sample interval
!  28 Oct 10 - HPATH needs binary header swapped on Intel
!  15 Oct 14 - The 11 May 06 change of nsecs preset from -1 to 0 ruined checks for big plots,
!              so change it from an error to a warning.
!            - Use si and delay from common
!  13 Jun 20 - Preset NSECS first thing to fake out all the presets elsewhere
!  22 Jun 20 - Changed COMMON /porder/ iorder(4) to same as contro iorder(100).  OSX cares.
!  1 Jul 20 - Add WARNING if both ANN and ANN2 are type HEADER (anntyp 11)
!
!
     ...
     *                  iptype
      ...
      LOGICAL IEXIST
      DIMENSION BUF(100),LBUF(100),IBUF(100),
     *          ISCR(1100),SCR(100),LSCR(10)
      INTEGER*2 IBUF,ISCR
     ...
     *            (NINC,LVALS(30)),
     ...
     6            (plotter,lvals(56)),
     7            (trim,lvals(57))
!    8            secs would be (58), but is nsecs is vals(2)
...
!      NS=0
      ...
      irectify = 0
      nraslines = 0
!**** set iplot=1 if   PLOT is in the PROCS list
      DO 10 i = 1, num
   ...
   10 CONTINUE
      ...
      itrim = 0
!****
!****   GET A DISC FILE TO HOLD THE PARAMETERS FOR THE EXECUTION PHASE
!****  PLOT uses FORTRAN I/O, not the usual SIOSEIS I/O
!****
      CALL GETFIL(2,MUNIT,token,ISTAT)
      IF( ICOMPT .EQ. 1) MUNIT = MUNIT + 12
      OPEN( unit = munit, FORM='UNFORMATTED', STATUS = 'SCRATCH' )
!****
!****  GET THE EBCDIC TAPE HEADER SQUARED AWAY
!****
...
!****     We can assume Sun rasterfiles won't be created on non Sun
!****     computers or with oddball plotters like the Printronix & CItoh
...
!**** don't upcase filenames!
      ...
      IF( names(nparam) .EQ. 'ANN2' ) THEN
          ...
          ENDIF
      ENDIF
      IF( anntyp .EQ. 11 .AND. anntyp2 .EQ. 11 ) THEN
          PRINT *,' *** WARNING  ***  ANN and ANN2 should not both be ',
     &          'HEADER.'
          iwarn = iwarn + 1
      ENDIF
...
!****             allow the gray level to be a color. Legal are 0-7.
                  IF( nchars .GT. 4 ) THEN
  ...
  500 CONTINUE                                                          !  32 BIT INTEGER VALUES
      ...
      GO TO 100
!****
!****    Do some additional preseting for different plotters!
!****
 1000 CONTINUE
!****  Preset NSECS first thing to fake out all the presets elsewhere pch 13 Jun 2020
!****  The "new" preset is the length of the first trace
!****
      IF( nsecs .LE. 0 ) nsecs = numdat * si
     ...
     &     'Use OPATH and program SIO2SUN for color Sun rasterfiles.'
      ...
      ENDIF
!**** Set up gray scale.
!**** (Watch out for 1 gray scale given!  e.g. colors 0 gray7)
      IF( ngray .GT. 0 ) THEN
...
!	6	 36  36  36         grey1
!	7	  0   0   0         gray0	black
!****     for sioseis plots, the bigger the number, the darker the plot
              DO i = 1, 8
                 colors(i) = 8 - i
           ...
           ENDIF
      ENDIF
!      IF(NIBS.NE.60) GO TO 1100                                         ! IS IT A PRINTRONIX 300
!      rnibs = 60.
!      OFFSET=.6                                                         ! NO ANNOTATION IS POSSIBLE
!      SIZE=13.5                                                         ! IT IS COMPUTER PAPER WIDE
!      IF(VSCALE.EQ.0.) VSCALE=4.17
!      DO 1010 I=1,20
!      IF(ABS(VSCALE-FLOAT(I)*2.085).LT..0001) GO TO 1030
! 1010 CONTINUE
!      PRINT 1020
! 1020 FORMAT(' ***  WARNING  ***  VSCALE SHOULD BE A MULTIPLE OF ',
!     *   '2.085 WHEN PLOTTING TIME DATA ON THE PRINTRONIX 300.')
!      IWARN=IWARN+1
! 1030 IF(NSECS.EQ.-1) NSECS=3.
!      GO TO 2000
 ...
 1500 CONTINUE
     ...
     &    nibs .EQ. 8425 .OR. nibs .EQ. 8625 ) THEN
          ...
          ELSE
              rnibs = 400.
          ENDIF
!****  Interesting problem on the Versatec 7000 series:
!****  The specs list the 7224 as 23.52, but that is WRONG
!****  The 7224 is 23.04, the 7225 is 23.52
          size = 23.04
...
!****  nsecs is now preset to 0
!****  with 600dpi, don't worry about samples being on rasters exactly.
          rnibs = 600.
          offset = 1.
          IF( vscale .EQ. 0 ) vscale = 2.5
     ...
     &    nibs .EQ. 2368 ) THEN
!****   850 has 1728 pixels, 9800 has 4096, 9315 has 2048
          rnibs = 203.
          offset = .8
          IF( nibs .EQ. 850 ) THEN
              size = 8.5
...
!****
 ...
 2340 CONTINUE
!****
!**** OFFSET must be >0 so indices do go negative in trplot
!      IF( itrim .GE. 1 .AND. itrim .LE. 3 ) offset = 0
     ...
     &        chart(2) .LT. .4 .OR. chart(2) .GT. 1 ) THEN
      ...
      ENDIF
!****
!****      Create the plot header - from the SEGY EBCDIC header
!****
...
!         make sure the Epson is set to Pica
          card(1:4) = CHAR(27) // '2  '
      ...
      ENDIF
!****
!****  Write the SEGY EBCDIC header to the plot header.
!****  Drop the last line and put the plot parameters there instead.
!****
      DO 3020 I=1, 40
          ...
          IF(IFTN.EQ.0) THEN                                            ! IS IT A FORTRAN WRITE? (I.E. THE PRINTRONIX OR VERSATEC)
             ...
             ELSE
!                CARD(80:80)=CHAR(15)                                   ! IT MUST BE A VERSATEC
     ...
     *                   card(80:80) = CHAR(10)                         !  Printronix 
          ...
          ENDIF
 3020 CONTINUE
!****  NOW WRITE 3 LINES OF PLOT INFO
 ...
 4000 CONTINUE
      ...
      IF( IDIR .EQ. -1 ) THEN                                           ! IF DIR is LTR
          ...
          IF( ncolors .NE. 0 .AND. colors(1) .NE. 8 ) THEN
           ...
           ENDIF
      ENDIF
!**** There's a weird color problem when a def is 0, so kludge around it
