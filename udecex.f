...
!     UDECEX IS THE EXECUTION PHASE OF THE SEISMIC REFLECTION PROCESS UDECON
!  (USER DECONVOLUTION).  THE USER'S PARAMETERS MUST BE IN
!  DISC FILE MUNIT (IN COMMON /UDECON/) AND THE TRACE WITH TRACE HEADER
!  MUST BE IN MEMORY ARRAY BUF.  DECON WINDOW TIMES FOR TRACES BETWEEN
!  THOSE SHOTS OR RPS DESCRIBED BY THE USER ARE CALCULATED BY LINEAR
!  INTERPOLATION.
!
!  ARGUMENTS:
!  SCR    - A SCRATCH ARRAY FOR READING THE PARAMETERS.
!
!  COPYRIGHTED (C) BY:
!  PAUL HENKART, SCRIPPS INSTITUTION OF OCEANOGRAPHY, AUGUST 1986
!
!   mod 17 Jun 10 - change the print output to be 1 value per line
!
!
...
!****
!****     FIND THE PARAMETER LIST (ON DISC)
!****
      IF( FIRST ) THEN
          FIRST=.FALSE.
...
!****
!****   set up the indices
!****   assume that if input is given, there is a desired output
!****   assume that if there is an autocorrelation there is a crosscorrelation
!****   assume that if there is an input, there is not an autocorrelatiobn
!****
      IF( IAND( lprint,2 ) .NE. 0 ) THEN
...
!****
     ...
     *             scr(iauto),nfpts)                                    ! do the autocorrelation
          ELSE
              ...
              CALL APWAIT
! ninpts must be GE ndesir+nfpts
      ...
      ENDIF
!****
!****    add in the prewhittener  (add a percentage of the 0 lag to the 0 lag)
!****
      IF( prewhi .NE. 0. ) THEN
          IF( iuseap .EQ. 0 ) THEN
      ...
      ENDIF
!****
!****   print the cross and auto correlation
!****
...
!      IF( iuseap .EQ. 0 ) THEN
!          CALL eureka( nfpts, scr(iauto), scr(icross), scr(ifilt),
!     *                 scr(iprede) )
          CALL wiener( nfpts, scr(iauto), scr(icross), scr(ifilt),
     *                scr(iprede), 1, ierr )
!      ELSE
!          CALL WIENER(nfpts,iauto,icross,ifilt,iprede,1)
!      ENDIF
!****
!****   print the filter if requested to do so
!****
...
!****
      ...
      ENDIF
!****
!****      DEconvolve the input (if it exists)
!****
      IF( ninpts .NE. 0 ) THEN
          IF( iuseap .EQ. 0 ) THEN
