...
!
!  mod 13 Apr 00 - Add IOFMT
!                - Add Hilbert and Analytic outputs
!  mod 15 Feb 05 - When analytic output, divide the sample interval by 2.
!  mod 24 Mar 05 - Adjust the integer sample interval by NINT of the real.
!  mod 6 Sep 05 - Redo error/warning messages about large sizes.
!               - Also treat numdat and header like 32 bits
C
...
! idtype = 2, frequency-wavenumber domain in rectangular coordinates
...
!        = 9, complex time domain or real Analytic trace
      COMMON /APMEM/AP(500000)                                           ! THE AP SIMULATOR DATA ARRAY
      COMMON /f2t/ osi, lprint, iofmt
! iofmt = 1 means real time domain,
!       = 2 means complex time domain, 
!       = 3 means Hilbert (real time domain)
!       = 4 means Analytic (complex time domain)
...
!     1 rad = 57.29578deg,    1 deg = .0174532rad
...
C
      ...
      STOP
   30 CONTINUE
!     iofmt = 3 means Hilbert, 4 means analytic
      IF( iofmt .EQ. 3 .OR. iofmt .EQ. 4 ) THEN
!****     Do Hilbert in nextad, save the input!
          ...
          ELSE
              DO i = 0, n2-1
                 ap(nextad+i) = ap(in+i)
              ENDDO
!             make the input rectanular coordinates in ap(in)
              CALL rectc( n, ap(in), ap(nextad), ap(nextad+n) )
              idtype = 4
          ENDIF
!**** the input trace is in rectangular coordinates in ap(in)
!**** the amplitude is in ap(nextad)
!**** The phase is in ap(nextad+n), subtract or add 90 deg or pi/2.
!**** depending on the sign.  Remember fftfwd put it in
!**** -nyqust to 0 to nyqust-deltaf order.  Leave 0 alone.
!**** See Bracewell, pg 267
          DO i = 0, no2-1
          ...
          DO i = no2, n2-1
             ap(nextad+n+i) = ap(nextad+n+i) + pio2
          ENDDO
!**** convert the Hilbert to rectangular in ap(nextad+n2)
          CALL rectc( n, ap(nextad+n2), ap(nextad), ap(nextad+n) )
          CALL fftinv( ap(nextad+n2), ipowr2 )
!**** the time domain quadrature is in ap(nextad+n2)
!**** the input frequency domain trace is in rectangular coordinates in ap(in)
          buf(49) = buf(49) / 2.
          ibuf(59) = NINT(buf(49) * 1000000. )
      ENDIF
!****
!****  Convert the input frequency domain trace to the time domain
!****
...
!****
      ...
      ENDIF
!****
!****  OFMT 3 = real time domain Hilbert or quadrature
!****
      IF( iofmt .EQ. 3 ) THEN
          j = 0
          DO i = 0, n
      ...
      ENDIF
!****
!****  OFMT 4 = real time domain Analytic (input, Hilbert pairs)
!****
      IF( iofmt .EQ. 4 ) THEN
!****    the real is in the evens (see type 1) and the imaginaries
!****    are in the odd.  Write over the odds with the Hilbert.
          DO i = 0, n2-1, 2
