...
!     GETPRO READS A USER'S LIST OF PROCESSES AND BUILDS ARRAY IORDER.
!  THE ORDER IN WHICH THE USER LISTS THE PROCESSES IS THE ORDER IN WHICH THE
!  PROCESSES WILL BE APPLIED TO THE DATA.
!     THE LIST OF LEGAL PROCESS NAMES IS COMMUNICATED VIA COMMON /PNAMES/ AND
!  THE ORDER OF PROCESSING IS A LIST OF PROCESS NUMBERS AND IS SET BY GETPRO
!  IN COMMON /PORDER/. THE NAMES EDIT AND EXEC ARE ALLOWED TO BE IN THE USER'S
!  LIST OF PROCESSES.  THE USER MUST SPECIFY EXEC IN ORDER TO GET THE JOB TO
!  EXECUTE AS WELL AS TO EDIT.  IF EDIT IS SPECIFIED OR EXEC IS NOT SPECIFIED,
!  THEN NO TAPES WILL BE HUNG AND THE JOB WILL NOT EXECUTE - THE JOB WILL THEN
!  BE AN EDIT ONLY JOB.
!     GETPRO READS A PROCESS NAME FROM A CARD IMAGE SUPPLIED BY THE USER AND IS
!  IN A PRIMOS BUFFER READ BY CONTRO.  THE PROCESS NAME IS CHECKED FOR VALIDITY
!  BY SCANNING THE ARRAY NAMES WHICH ALSO HAS THE PROCESS NUMBER IMPLIED THROUGH
!  IT'S ORDER IN THE ARRAY.  THIS NUMBER IS THEN PUT INTO THE IORDER ARRAY.  THE
!  USER'S LIST OF PROCESSES MUST BE TERMINATED BY 'END', UPON WHICH GETPRO
!  RETURNS TO CONTRO.
!
! PAUL HENKART, SCRIPPS INSTITUTION OF OCEANOGRAPHY, FEBRUARY 1979
!
!mod 13 July 99 - Add exemption for diskoe-diskoj
! mod  May 00 - Also, set all processes to require parameters and only set
!               the ones that do not to 0.
! mod 2 Feb 07 - Check for duplicate names in the PROCS list.
! mod 18 Oct 10 - Check for syn, diskin, segddin being in the same list
!
      PARAMETER (NAMESS=100)
      CHARACTER*7 NAMES(NAMESS)
      DIMENSION LENGTH(1111),ITYPE(1)
      CHARACTER*10 CTOKEN
      COMMON /PORDER/ NUM,IORDER(NAMESS)
      COMMON /pnames/ pnames
!****  beware of length of pnames since chkprc uses it!
      CHARACTER*7 pnames(namess)
!    iok array is to signal that a process' parameters are required
...
!***
   ...
   20 FORMAT(' <  ENTER PARAMETERS  >')
      ...
      GO TO 10
!****
!****    GOT A TOKEN  -  IS IT A VALID PROCESS NAME?
!****
   30 CONTINUE
...
!****     BECAUSE THE EDIT SETS THE PRESETS.  ALSO THE EXECUTE REQUIRES THAT
!****     THE PARAMETER FILE EXIST.
...
!     32 is fk2tx - no params needed
!****  Geez, that's stupid.   Set them all to 1 and just set the ones that
!**** don't require parameters to 0.
      DO i = 33, namess
         IF( j .EQ. i ) iok(j) = 1
      ENDDO
      iok(35) = 0                                                         ! F2T 
