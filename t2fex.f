...
!     T2FEX IS THE EXECUTION PHASE OF THE SEISMIC PROCESS T2F, TIME TO FREQUENCY
!  TRANSFORMATION.  THE TRANSFORMATION IS DONE BY THE FFT IN EITHER THE AP
!  OR HOST MEMORY, DEPENDING ON THE IUSEAP SWITCH.  IN BOTH CASES, THE DATA IS
!  FIRST WINDOWED, THEN FILLED WITH ZEROES TO A POWER OF 2, THEN THE FFT, THEN
!  SCALED DOWN BY 1./FFTLEN.  THE SEGY TAPE HEADER SIGNAL FOR DATA TYPE IS SET
!  TO 'F' FOR RECTANGULAR COORDINATES OR 'W' FOR POLAR COORDINATES.
!
!
!  ARGUMENTS:
!    BUF,LBUF,IBUF   - THE TRACE HEADER FOLLOWED BY THE TRACE
!    SCR,LSCR,ISCR   - A SCRATCH ARRAY
!
!  WRITTEN AND COPYRIGHTED BY:
!  PAUL HENKART, SCRIPPS INSTITUTION OF OCEANOGRAPHY, 14 JANUARY 1984
...
!
!  mod 25 Aug 93 - Increase maxdow from 100 to 1000
!  mod 13 Oct 93 - Save the input (time domain) sample interval.
!  mod 22 Dec 97 - Change MAX0 to MAX
!  mod 6 Sept 05 - Work on overflow messages.
!  mod 25 Jul 06 - Use numdat rather than ibuf(58) so we get > 32K samples
!  mod 12 Jan 07 - Index for zero pad was off by 1 (didn't zero nsamps + 1)
!                - The window was not created correctly (bad arg to window)
!  mod 2 Oct. 2008 - Add coords AMPL
!
!
...
!****
      ...
      GO TO 70
!****
!****    NOW FIND THE PARAMETERS FOR THE  TRACE NUMBER
!****
...
!
 ...
 1050 CONTINUE
      ...
      in = (stime - delay) / si + 1
!****  AND MAKE SURE THE AP ADDRESSING ALLOWS FOR LARGER DATA ARRAYS
      IF(IUSEAP.EQ.1) GO TO 1500                                        ! IS THERE AN AP?
!****
!****    DO THE TRANSFORMATION IN THE HOST
!****
      IF(IWINDO.EQ.5.OR.NWINDO.LE.0) GO TO 1200                         ! DON'T DO THE WINDOW IF IT IS A RECTANGULAR WINDOW
      IF(NW.NE.NWINDO) CALL WINDOW(-IWINDO,WINDO,nwindo,DUMMY)
      DO I=1,NW                                                    ! WINDOW RETURNED EITHER THE LEFT HALF OF THE WINDOW OR ALL OF IT
 1110    A(IN+I-1)=A(IN+I-1)*WINDO(I)                                      ! APPLY THE WINDOW!
      ENDDO
!      IF(IWINDO.GT.0) GO TO 1200                                       ! WAS THAT THE WHOLE WINDOW WE JUST DID?
...
!      GO TO 1200
! 1150 CONTINUE
!      IF( nw .LT. 10 .OR. nw .GT. maxdow ) THEN
!          PRINT *,' ***  ERROR  ***  Bad window length of ',nw
!          STOP
!      ENDIF
!      CALL WINDOW(-IWINDO,SCR,NW,DUMMY)                                ! THE WINDOW COULDN'T FIT IN THE WINDO ARRAY
!      IF( IAND(lprint,2) .NE. 0) PRINT *,' CALL window(',iwindo,nw
!      DO 1160 I=1,NW                                                   ! NOW APPLY THE NEWLY CREATED WINDOW
! 1160 A(IN+I-1)=A(IN+I-1)*SCR(I)
!      IF(IWINDO.GT.0) GO TO 1200                                       ! DID WE DO THE WHOLE WINDOW?
!      ISTART=NSAMPS+IN-NW-1                                            ! FIND THE BACK OF THE TRACE TO WINDOW
!      DO 1170 I=1,NW
! 1170 A(ISTART+I)=A(ISTART+I)*SCR(NW-I+1)
! 1200 IF(NWINDO.GT.0) NSAMPS=NWINDO                                    ! IF THE WHOLE WINDOW, CHANGE THE NUMBER OF SAMPLES
 ...
 1310    A(IN+I-1)=A(NEXTAD+I-1)*SCALE                                     ! SCALE THE DATA WHILE MOVING IT BACK TO IN
      ...
      IF( lcoord .EQ. 2 .OR. lcoord .EQ. 3 ) THEN
          DO i = 1, nsamps                                         ! move the data to nextad
 1400        a(nextad+i-1) = a(in+i-1)
          ENDDO
!****  NOTE THAT THE AMPLITUDE SPECTRUM PRECEDES THE PHASE SPECTRUM
...
!*****
 5000 CONTINUE
      NUMDAT=NSAMPS
!      IBUF(58)=NSAMPS
!      IF( ibuf(57) .EQ. 0 ) lbuf(29) = nsamps
