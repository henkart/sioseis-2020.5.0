     ...
     *           nready )
!  Split-Step migration by Dan Lizarralde
!  Copyright (C) by Woods Hole Oceanographic Institution, 1995
!
!  mod Apr 96 by gmk for:
!    smooth the time data
!    smooth the slowness in depth
!    write SEGY velocity file
!  mod May 96 by pch to error out if nx * nz > iapsiz because Velocities
!      are stored there.
!  mod 9 Apr 09 - Use unsigned int for header number of samples
!
...
!***  Zero Arrays       
   ...
   60    scr(i) = 0.
      ENDDO
!     fastf( reals, imaginaries, number which is a power of 2)
...
!****
  ...
  210                   vels(i) = vels2(i)
             ...
             ENDIF
             IF( vintpl .EQ. 1 ) THEN
!***             Velocity variation is by iso-velocity
                 DO 310 i = 1, nvels1, 2
                    v = vels1(i)
                    d = vels1(i+1)
                    DO 290 j = 1, nvels2, 2
...
!****            Velocity interpolation by iso-depth
...
!                ibuf(isampptr) = nz   
  ...
  500     CONTINUE
          ...
          DO 700 i = 1, nz
             index = i
             DO 600 j = 1, nx
!****           transpose and convert from meters to kilometers
...
!**** The data now sits on disk as REAL depth slices.
      ...
      IF( nx * nz .GT. isize ) THEN
          big = .TRUE.
!***GMK If lunt is not opened in sspost2 (since big is defined differently) open it now
