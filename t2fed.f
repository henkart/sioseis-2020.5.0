...
!                                 ------- ---
!
!  DOCUMENT DATE: 11 JANUARY 1987                                             69
!
!       PROCESS T2F CONVERTS TRACES FROM THE TIME DOMAIN TO THE FREQUENCY
!  DOMAIN.  EACH TRACE IS WINDOWED, PADDED WITH ZEROES TO A POWER OF 2,
!  TRANSFORMED, AND OPTIONALLY CONVERTED TO POLAR COORDINATES.  TRACES LEFT IN
!  RECTANGULAR COORDINATES ARE IN COMPLEX FORM SUCH THAT THE IMAGINARY PART
!  OF EACH FREQUENCY IMMEDIATELY FOLLOWS THE REAL (REAL-IMAGINARY PAIRS).
!  TRACES IN POLAR FORM HAVE THE FREQUENCY SPECTRUM IN THE FIRST HALF OF THE
!  OUTPUT TRACE WHILE THE WRAPPED PHASE SPECTRUM IS IN THE SECOND HALF OF THE
!  TRACE.
...
!  PROCESSES FILTER AND DECON UNTIL THE FREQUENCY DOMAIN PROCESSES ARE WRITTEN.
...
!  --- --------- ----------
!  STIME  - START TIME, IN SECONDS, OF THE DATA TO TRANSFORM.  The use of STIME
!           causes the the deep water delay to be set to 0. and the first time
!           sample on an inverse transform (PROCESS F2T) to be 0.
!           PRESET = THE DELAY OF THE TRACE.  E.G.  STIM3 3.3
...
!                THE USE OF RECTANGULAR COORDINATES IS SLIGHTLY FASTER THAN
!                POLAR COORDINATES, AND IS UNDERSTOOD BY FREQUENCY DOMAIN
!                PROCESSES SUCH AS F2T.
!         =POLR, POLAR CORDINATES.  THE FIRST HALF OF THE OUTPUT TRACE WILL BE
!                THE AMPLITUDE SPECTRUM AND THE SECOND HALF OF THE TRACE WILL
!                BE THE PHASE SPECTRUM.
...
!****
 ...
 1050 CONTINUE
      ...
      ENDIF
!      IF( winlen .EQ. 0 ) THEN
!          PRINT *,' ***  WARNING  ***  WINLEN not given.'
!          iwarn = iwarn + 1
!      ENDIF
      LSCR(1)=FNO
      LSCR(2)=LNO
