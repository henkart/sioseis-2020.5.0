...
!  mod 2 Apr 15 - HIRES problem (mixing time and indices)
!               - HIRES was garbage when data had delay
!               - HIRES indexing was off
!  mod 18 Jul 18 - Swap the velocity values when writing a segy OPATH
!                - Also don't requite lprint to be used to make OPATH to work
!
      PARAMETER ( NPARS = 18 )
...
!****     swap it back for the next usage
              IF( icompt .EQ. 2 .OR. icompt .EQ. 4)
     *            CALL swp_trhdr( ibuf, lbuf )
              IF( icompt .EQ. 2 .OR. icompt .EQ. 4)
...
!  lscr(1) starts the array of indices fo the sample closes to the TX we want
          DO i = 1, nsamps
             a(indextx+i-1) = FLOAT(lscr(i)) * si + delay
          ENDDO
!  a(indextx) starts the arrary of times of the nearest sample of the TX we want
!  suppose we could just divide the REAL TX (nmonap returned it in (nextad)
          DO i = 1, n2
             buf(numhdr+i) = 0.
          ENDDO
          DO i = n2+1, nsamps-n2
             ...
             ELSE
!  a(indextx) starts the arrary of times of the nearest sample of the TX we want
!  a(nextad) has the REAL times we really want - seems to be off a samples
!  a(indata+1) has the trace
