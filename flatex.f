...
!     FLATEX IS THE EXECUTION PHASE OF THE SEISMIC REFLECTION PROCESS FLATEN..
!  THE USER'S PARAMETERS MUST BE IN DISC FILE MUNIT (IN COMMON /FLATEN/) AND 
!  THE TRACE WITH TRACE HEADER MUST BE IN MEMORY ARRAY BUF. 
!
!  ARGUMENTS:
...
!           BE AT LEAST 7 32BIT WORDS BIG.  SCR MAY BE DESTROYED BY THE CALLING
!           ROUTINE.
...
!****  the Sea Beam puts depth (in meters) into the header  It doesn't do
!****  this occasionally (at the beginning of a SeaBeam file)
 ...
 1130                a(in+n+i-1) = 0.
          ...
          ELSE
              ...
              IF( in .EQ. 0 ) THEN
                  DO i = 1, n
                  buf(numhdr+nsamps-i+1) = buf(numhdr+nsamps-nshift-i+1)
                  ENDDO
                  DO i = 1, nshift
 1170                buf(numhdr+i) = 0.
                  ENDDO
