...
!
!       Process FKSHIFT performs an extrapolation via a phase shift in 
!  the FK domain.  FKSHIFT is depth migration of a horizontally layered
!  media whose velocity is always increasing.
!       The data must have been transformed into the FK domain prior to c  process FKSHIFT and it is left in the FK domain.  Use process TX2FK 
!  prior to FKSHIFT and FK2TX after FKSHIFT.
!       FKSHIFT can be used to forward extrapolate shot gathers by
!  specifying the true velocity and extrapolation height (thickness).
!       FKSHIFT can be used to backwards extrapolate stacked data to thec  seafloor by halving the specified velocity and specifying the output
!  time delay.
!
!       Only one parameter may be given.
!
...
!  DELTAT  - The sample interval of the data in the time domain, in
!            seconds.
!            Preset = SEGY header.    e.g.  deltat .004
