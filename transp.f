...
!****   The first word in file lun is at word 1, not word 0
      IF( first ) THEN
          first = .FALSE.
          CALL podisc( lun, 1, 0 )
!         most systems make you write to the file before you can position them
          DO 300 i = 1, nsamps
...
!       print *,' write ',n,' words at ',ipos,' index=',index,
!     *      ' dat=',t(index),t(index+1),t(index+2)
             index = index + maxc
  500     CONTINUE
...
!             IF( istat .NE. n ) THEN
!                 PRINT *,' disc read error, ipos=',ipos,' index=',
!     *                   index,' n=',n,' nsamps=',nsamps,' nx=',nx,
!     *                   ' nsofar=',nsofar,' maxcol=',maxcol
!             ENDIF
             index = index + n
  800     CONTINUE
          icol = 1
...
!****  WATCH OUT!  lun starts at word address 0
      IF( first ) THEN
          first = .FALSE.
          CALL podisc( lun, 1, 0 )
!         most systems make you write to the file before you can position them
...
!       print *,' write ',n*2,' words at ',ipos,' index=',index,
!     *      ' dat=',t(index),t(index+1),t(index+2)
             index = index + maxc
 1500     CONTINUE
          nsofar = nsofar + maxcol
...
!             IF( istat .NE. n ) THEN
!                 PRINT *,' disc read error, ipos=',ipos,' index=',
!     *                   index,' n=',n*2,' ncsamps=',ncsamps,' nx=',nx,
!     *                   ' nsofar=',nsofar,' maxcol=',maxcol
!             ENDIF
             index = index + n
 1800     CONTINUE
