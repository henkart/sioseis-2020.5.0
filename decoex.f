...
!     SDECEX IS THE EXECUTION PHASE OF THE SEISMIC REFLECTION PROCESS SDECON
!  (TIME VARYING SPIKING DECONVOLUTION).  THE USER'S PARAMETERS MUST BE IN
!  DISC FILE MUNIT (IN COMMON /SDEC/) AND THE TRACE WITH TRACE HEADER
...
c  MODIFICATIONS:
!  mod 4 Feb 11 - Add igap and pass to subroutine DECON
...
!****   WHICH ARE EACH FILTER LENGTH + PREDICTION DISTANCE LONG
...
!****
!****   MULTIPLY THE FRONT OF THE APPLICATION WINDOW WITH THE LAST WINDOW'S
!****   REAR MERGE ZONE RAMP.  THE RAMP IS STILL IN THE AP.  ADD THE MERGE ZONE
!****   TO THE MERGE ZONE THAT IS ALREADY IN THE OUTPUT BUFFER.
!****
      IF( I .NE. 1 ) THEN
c****     n is set on the first app window and is the whole app window
      ...
      ENDIF
!****
!****    MOVE THE APPLICATION WINDOW TO OUTPUT BUFFER
!****
      IF(I.EQ.1) N=NPTS
 ...
 1475 IF( I .EQ. NDOWS ) GOTO 1500
      ...
      IANSW = IANSW + N
!****
!****  BUILD A RAMP FOR THE MERGE ZONE, AND APPLY IT TO THE BACK END MERGE
!****  AREA AND PUT THIS BACK END MERGE ZONE INTO THE OUTPUT ARRAY
!****
      ...
      ELSE
          ...
          DO II = 1, N
             A(J+II) = FLOAT(II) * TEMP
          ENDDO
          K = IRESUL - 1
          J = IRAMP + N
          M = IANSW - 1
          DO II = 1, N
             A(M+II) = A(K+II) * A(J-II)
          ENDDO
      ENDIF
 1500 CONTINUE
c**** 
